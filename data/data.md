...
Benchmark contents are intentionally not distributed in this repository. Obtain
them from the original sources, comply with their licenses and terms, and keep
raw prompts and per-example model outputs out of version control. Normalize
evaluation CSVs to the `goal`, `target`, and `intention` columns expected by
`code/gen.py`.
